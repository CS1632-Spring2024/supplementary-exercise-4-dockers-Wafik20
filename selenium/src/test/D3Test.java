...
public class D3Test {
  ...
  public void dEFECT1FUNFEED() {
    WebElement FeedACatLink = driver.findElement(By.linkText("Feed-A-Cat"));
    FeedACatLink.click();

    WebElement feedCatTextbox = driver.findElement(
      By.xpath("//input[@id='catnips']")
    );
    feedCatTextbox.sendKeys("0");

    WebElement feedButton = driver.findElement(
      By.xpath("//button[contains(.,'Feed')]")
    );
    feedButton.click();

    WebElement feedResult = driver.findElement(By.id("feedResult"));
    String feedResultText = feedResult.getText();

    assertEquals("Cat fight!", feedResultText);
  }

  @Test
  public void dEFECT2FUNGREETACAT() {
    // Set test precondition
    js.executeScript(
      "javascript:{document.cookie = \"1=true\";document.cookie = \"2=true\";document.cookie = \"3=true\";void(0)}"
    );

    WebElement GreetACatLink = driver.findElement(By.linkText("Greet-A-Cat"));
    GreetACatLink.click();

    // find the element where the greeting is displayed
    WebElement greeting = driver.findElement(By.id("greeting"));
    String greetingText = greeting.getText();

    // Check if greeting contains "Meow!"
    assertTrue(
      "at least one 'Meow!' is displayed!",
      !greetingText.contains("Meow!")
    );
  }
  @Test
  public void dEFECT3FUNGREETACATWITHNAME() {
    ...
    driver.get("http://localhost:8080/greet-a-cat/Jennyanydots");
    assertEquals("Jennyanydots is not here.",driver.findElement(By.xpath("//div[@id=\'greeting\']/h4")).getText());
  }
}